const users = [
  ...
  users.forEach(function(chacha) {
    sum+=`<div class="main">
        <img class="cov" src="${chacha.coverPhoto}"alt="">
        <div class="mid">
            <img  src="${chacha.profilePhoto}"alt="">
        </div>
        
        <h2>${chacha.fullName}</h2>
        <p>${chacha.description}</p>
         <img class="under" src="https://png.pngtree.com/png-vector/20220518/ourmid/pngtree-star-line-divider-silhouette-png-image_4635447.png" alt="">
        
        <div class="foll">
        
            <h1>${chacha.mediaUploadedCount} <br>media</h1>
            <h1>${chacha.followersCount} <br>follwers</h1>
            <h1>${chacha.followingCount} <br> following</h1>
        </div>
    </div>`
